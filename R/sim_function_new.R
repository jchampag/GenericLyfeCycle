##################################################
#Function: population dynamics simulation based on input data
...
#' @traj  the number of trqjectories you want (only if process errors are used)
#' @param_R how Beverton-Holt stock recruitement relqtionship is parametrized: in steepness (h) or with alpha
...
"life_cycle_sim" <- function(data,traj= 1,param_R="h",age.plus=FALSE){
  ...
  # create Process Error vectors not used in the paper
  # pe_R <- array(rnorm(mean = 0,sd=data$sd_R,n=length(data$years)*length(data$nurseries)*max(traj)),
  #                      dim=c(length(data$years),length(data$nurseries),max(traj)),
  #                      dimnames = list(data$years,data$nurseries,1:max(traj)))#
  # 
  # pe_Sjuv <-  array(rnorm(mean = 0,sd=data$sd_Sjuv,n=length(min(data$ages):data$age_sa)*length(data$years)*length(data$nurseries)*max(traj)),
  #                          dim=c(length(min(data$ages):data$age_sa),length(data$years),length(data$nurseries),max(traj)),
  #                          dimnames = list(c(min(data$ages):data$age_sa),data$years,data$nurseries,1:max(traj)))#
  # 
  # pe_Sad <-  array(rnorm(mean = 0,sd=data$sd_Sjuv,n=length(data$ages)*length(data$years)*length(data$regions)*max(traj)),
  #                         dim=c(length(data$ages),length(data$years),length(data$regions),max(traj)),
  #                         dimnames = list(data$ages,data$years,data$regions,1:max(traj)))#
    ...
    for (y in 1:max(data$years)){
      ...
      for (a in 1:(max(data$ages)-min(data$ages)+1)){
        ...
        if(a <data$age_sa-min(data$ages)+1){ #for fish of age < age_subadult 
          ...
          #####################
          ##juveniles arrival from nurseries
          #####################
          ...
          if(a<(data$age_mat-min(data$ages)+1)){  #for non mature fish, no movement on spawning grounds
            ...
            if(a==(data$age_sa-min(data$ages)+1)&data$age_sa==data$age_mat){ #special case for those fish
              ...
              for (r in seq_along(data$regions)){
                ...
                # add subadult fish arriving in N_region_af_mvt  => will move to spawning grounds
            ...
            for(j in seq_along(data$spwn_grd)){
              ...
              if(a==(max(data$ages)-min(data$ages)+1)){
                ...
                Eggs[y,j,s] <- sum(N_spwn_grd_bf[,y,j,s]*data$M_spwgrd_surv*
                ...
                SSB[y,j,s] <- sum(N_spwn_grd_bf[,y,j,s]*data$SwatA*data$mat*data$M_spwgrd_surv*
              ...
              N_spwn_grd_end[a,y,j,s] <- N_spwn_grd_bf[a,y,j,s]*data$M_spwgrd_surv*
                exp(-data$ts_sp/365*(data$F_spwn_grd[a,y,j]+data$M_spwn_grd[a,y,j])) 
              
              #compute catches
              C_spwn_grd[a,y,j,s] <- N_spwn_grd_bf[a,y,j,s]*data$M_spwgrd_surv*  
